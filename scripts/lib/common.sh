...
# CodeSeek common script library.
...
build_codeseek() {
  local output="${1:?build_codeseek: output path required}"
  log "Building CodeSeek"
  (
    cd "$ROOT_DIR"
    go build -o "$output" ./cmd/codeseek
  ) 2>&1 | tee -a "$LOG_FILE"
}
...
wait_for_server() {
  ...
  while (( SECONDS < deadline )); do
    if ! kill -0 "$SERVER_PID" >/dev/null 2>&1; then
      log_error "CodeSeek exited before it became ready on ${BASE_ADDR}"
      return 1
    fi
    if (echo > "/dev/tcp/${HOST}/${PORT}") >/dev/null 2>&1; then
      return 0
  ...
  log_error "CodeSeek did not start on ${BASE_ADDR}"
  return 1
}

cleanup_server() {
  if [[ -n "${SERVER_PID:-}" ]] && kill -0 "$SERVER_PID" >/dev/null 2>&1; then
    log "Stopping CodeSeek"
    kill "$SERVER_PID" >/dev/null 2>&1 || true
    wait "$SERVER_PID" >/dev/null 2>&1 || true
    log "CodeSeek stopped"
  fi
}
...
start_server_background() {
  log "Starting CodeSeek on ${BASE_ADDR}"
  log "CodeSeek log: ${LOG_FILE}"
  (
    cd "$ROOT_DIR"
...
load_env_file() {
  ...
  if [[ -f "$env_file" ]]; then
    ...
    log_error "codeseek env file not found at ${env_file}"
    exit 1
  fi
  if [[ -z "${CODESEEK_ADDR:-}" ]]; then
    log_error "codeseek not configured in ${env_file}"
    exit 1
  fi
  parse_addr "$CODESEEK_ADDR"
}

verify_codeseek_alive() {
  if ! (echo > "/dev/tcp/${HOST}/${PORT}") >/dev/null 2>&1; then
    log_error "CodeSeek not reachable on ${HOST}:${PORT}"
    exit 1
  fi
}
...
validate_mode() {
  ...
  log_error "codeseek mode must be one of: ${allowed[*]}, got: ${current}"
  exit 1
}
...
env["ANTHROPIC_AUTH_TOKEN"] = "codeseek-proxy-placeholder"
...
if effective_model:
    lines.append(f"export CODESEEK_EFFECTIVE_CLAUDE_MODEL={shlex.quote(str(effective_model))}")
env_path.write_text("\n".join(lines) + "\n")
