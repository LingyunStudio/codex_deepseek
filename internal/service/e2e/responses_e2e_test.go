...
func TestResponsesTextE2E(t *testing.T) {
	...
	response := postResponses(t, handler, map[string]any{
		...
		"input":             "Reply with the words CodeSeek e2e ok.",
...
func TestResponsesPromptCacheE2E(t *testing.T) {
	e2eConfig := loadE2EConfig(t)
	if os.Getenv("CODESEEK_E2E_CACHE") != "1" {
		t.Skip("set CODESEEK_E2E_CACHE=1 to run cache-costing e2e")
	}
	...
	longContext := strings.Repeat("CodeSeek cache prefix stability sentence. ", 900)
	request := map[string]any{
		"model":             e2eConfig.ModelAlias,
		"instructions":      longContext,
		"input":             "Answer with one short sentence.",
		"prompt_cache_key":  "codeseek-e2e-cache",
...
func loadE2EConfig(t *testing.T) e2eConfig {
	...
	configPath := os.Getenv("CODESEEK_CONFIG")
	if configPath == "" {
...
func e2eModelAlias(routes map[string]config.RouteEntry) (string, error) {
	...
	if route, ok := routes["codeseek"]; ok && route.Model != "" {
		return "codeseek", nil
	}
