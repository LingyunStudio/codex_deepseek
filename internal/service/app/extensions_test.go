...
import (
	...
	"codeseek/internal/db"
	dbd1 "codeseek/internal/extension/db/d1"
	dbsqlite "codeseek/internal/extension/db/sqlite"
	"codeseek/internal/extension/deepseek_v4"
	kimiworkaround "codeseek/internal/extension/kimi_workaround"
	"codeseek/internal/extension/plugin"
	"codeseek/internal/extension/visual"
	"codeseek/internal/format"
)
...
func TestPluginsReadCurrentRuntimeConfig(t *testing.T) {
	...
	runtimeCfg.Routes["codeseek"] = config.RouteEntry{
		Provider: "anthropic",
		Model:    "claude-3-5-sonnet",
		Extensions: map[string]config.ExtensionSettings{
	...
	if !dsPlugin.EnabledForModel("codeseek") {
		t.Fatal("deepseek_v4 should read enabled state from current config")
	}
	rewritten := dsPlugin.RewriteMessages(&plugin.RequestContext{ModelAlias: "codeseek"}, []format.CoreMessage{{
		Role:    "user",
	...
	if !kimiPlugin.EnabledForModel("codeseek") {
	...
	limited := kimiPlugin.RewriteMessages(&plugin.RequestContext{ModelAlias: "codeseek"}, kimiMsgs)
	...
	if !visualPlugin.EnabledForModel("codeseek") {
		t.Fatal("visual should read enabled state from current config")
	}
	visCfg, enabled := visual.ConfigForModelFromResolvedConfig(runtimeCfg, "codeseek")
