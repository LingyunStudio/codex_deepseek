...
func (server *Server) handleResponses(writer http.ResponseWriter, request *http.Request) {
	...
	// Reject WebSocket upgrade requests so Codex falls back to HTTP.
	if strings.EqualFold(request.Header.Get("Upgrade"), "websocket") {
		log.Debug("拒绝 WebSocket 升级请求")
		writeOpenAIError(writer, http.StatusUpgradeRequired, openai.ErrorResponse{Error: openai.ErrorObject{
			Message: "WebSocket is not supported. Use HTTP POST instead.",
	...
	if err := json.Unmarshal(body, &responsesRequest); err != nil {
		log.Warn("无效的 JSON 请求体", "error", err, "body", string(body[:min(len(body), 300)]))
		payload := openai.ErrorResponse{Error: openai.ErrorObject{
