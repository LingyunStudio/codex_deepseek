...
//
// It is shared by internal/server and cmd/codeseek to produce
...
func GenerateConfigToml(output io.Writer, modelAlias string, baseURL string, codexHome string, providerCfg config.ProviderConfig, pluginCfg config.PluginConfig, serverCfg config.ServerConfig) error {
	...
	fmt.Fprintln(output, `model_provider = "codeseek"`)
	...
	fmt.Fprintln(output)
	fmt.Fprintln(output, "[model_providers.codeseek]")
	fmt.Fprintln(output, `name = "CodeSeek"`)
