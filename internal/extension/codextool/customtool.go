	...
	return []format.CoreTool{
		{
			...
			Description: "Create one new file. CodeSeek reconstructs the raw Codex apply_patch grammar from the result.",
			InputSchema: ApplyPatchSingleOpSchema("add_file"),
		},
		{
			Name:        ApplyPatchToolName(name, "delete_file"),
			Description: "Delete one file. CodeSeek reconstructs the raw Codex apply_patch grammar from the result.",
		...
		},
		{
			...
			Description: "Edit files by providing structured JSON patch operations. CodeSeek reconstructs the raw Codex apply_patch grammar from the result.",
