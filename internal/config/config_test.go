...
import (
	...
	deepseekv4 "codeseek/internal/extension/deepseek_v4"
	"codeseek/internal/extension/visual"
...
func TestXDGDefaultConfigPathUsesXDGConfigHome(t *testing.T) {
	...
	want := filepath.Join(configHome, "codeseek", "config.yml")
	if got != want {
...
func TestXDGDefaultConfigPathFallsBackToHome(t *testing.T) {
	...
	want := filepath.Join(configHome, ".config", "codeseek", "config.yml")
	if got != want {
		t.Fatalf("XDGDefaultConfigPath() = %q, want %q", got, want)
	}
}
...
`), config.LoadOptions{ExtensionSpecs: builtinExtensionSpecsForTest()})
	...
	if !cfg.ExtensionEnabled(deepseekv4.PluginName, "codeseek") {
		t.Fatalf("ExtensionEnabled(deepseek_v4, codeseek) = false, want true")
	}
	if cfg.ExtensionEnabled(deepseekv4.PluginName, "image") {
		t.Fatalf("ExtensionEnabled(deepseek_v4, image) = true, want false")
	}
	if cfg.RouteFor("codeseek").DefaultReasoningLevel != "high" {
		t.Fatalf("RouteFor(codeseek).DefaultReasoningLevel = %q", cfg.RouteFor("codeseek").DefaultReasoningLevel)
	}
	if got := len(cfg.RouteFor("codeseek").SupportedReasoningLevels); got != 2 {
		t.Fatalf("RouteFor(codeseek).SupportedReasoningLevels len = %d", got)
	}
...
`), config.LoadOptions{ExtensionSpecs: builtinExtensionSpecsForTest()})
	...
	if !cfg.ExtensionEnabled(visual.PluginName, "codeseek") {
		t.Fatal("ExtensionEnabled(visual, codeseek) = false, want true")
	}
	...
	resolved, _ := cfg.ExtensionConfig(visual.PluginName, "codeseek").(*visual.Config)
	if resolved == nil {
		t.Fatal("ExtensionConfig(visual, codeseek) = nil")
	}
	if resolved.Provider != "kimi" || resolved.Model != "kimi-vision" {
		t.Fatalf("ExtensionConfig(visual, codeseek) = %+v", resolved)
	}
...
func TestDefaultModelAliasFallsBackToMoonbridge(t *testing.T) {
	cfg := config.Config{Routes: map[string]config.RouteEntry{
		"codeseek": {Provider: "default", Model: "claude-test"},
		"other":      {Provider: "default", Model: "claude-other"},
	}}
	if got := cfg.DefaultModelAlias(); got != "codeseek" {
		t.Fatalf("DefaultModelAlias() = %q", got)
	}
}
...
func TestCodexModelUsesDefaultModelInTransform(t *testing.T) {
	...
	if got := cfg.CodexModel(); got != "codeseek" {
		t.Fatalf("CodexModel() = %q", got)
	}
}
