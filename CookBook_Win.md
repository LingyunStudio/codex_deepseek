...
| 2 | [把 Codex CLI 接上 CodeSeek](#2-把-codex-cli-接上-codeseek) | 3 min | ⭐⭐ |
...
## 2. 把 Codex CLI 接上 CodeSeek

**效果：** Codex CLI 走 CodeSeek 调用 DeepSeek。
...
CodeSeek 自带 Codex 配置生成器。先确认它在运行：
...
```powershell
$MODEL = go run ./cmd/codeseek -print-codex-model -config "./config.yml"
```

```powershell
go run ./cmd/codeseek -print-codex-config "$MODEL" -codex-base-url "http://127.0.0.1:38440/v1" -codex-home "$CODEX_HOME_DIR" -config "./config.yml" Set-Content -Path "$CODEX_HOME_DIR\config.toml" -NoNewline
```
...
**验证：** Codex 正常启动，提问后 CodeSeek 终端出现 `POST /v1/responses` 日志。

**搞不定：**

| 问题 | 原因 | 解决 |
|------|------|------|
| `connection refused` | CodeSeek 没启动 | 先跑菜谱 1 |
...
重启 CodeSeek（Ctrl+C 停掉，再 `go run`），curl 测试。
...
```powershell
go run ./cmd/codeseek -config /path/to/config.yml 2>&1 | Select-Object -First 30
```
...
没输出则 CodeSeek 未运行；有输出但请求失败则检查 model 名字。
