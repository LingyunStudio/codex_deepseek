...
CodeSeek 是一个 Go 语言编写的 HTTP 代理/协议转换服务器。对外暴露 **OpenAI Responses API**（`/v1/responses`），对内支持 **Anthropic Messages**、**Google Gemini（GenAI）**、**OpenAI Chat Completions** 四种上游协议，以及 OpenAI Responses 直通。
...
协议间工具调用的核心挑战在于格式差异。CodeSeek 的 `CoreTool` / `CoreContentBlock` 作为中间表示屏蔽差异：
